...
function mdg_head_cleanup() {
	...
	if ( !class_exists( 'WPSEO_Frontend' ) ) {
		...
		add_action( 'wp_head', 'mdg_rel_canonical' );
	} // if()
} // mdg_head_cleanup()
...
add_action( 'init', 'mdg_head_cleanup' );
 ...
 */
function mdg_language_attributes() {
	$attributes = array();
	$output = '';
	...
	$output = apply_filters( 'mdg_language_attributes', $output );

	return $output;
} // mdg_language_attributes()
add_filter( 'language_attributes', 'mdg_language_attributes' );
...
add_filter( 'wp_title', 'mdg_wp_title', 10 );
 ...
 */
function mdg_clean_style_tag( $input ) {
	preg_match_all( "!<link rel='stylesheet'\s?(id='[^']+')?\s+href='(.*)' type='text/css' media='(.*)' />!", $input, $matches );
	// Only display media if it is meaningful
	$media = $matches[3][0] !== '' && $matches[3][0] !== 'all' ? ' media="' . $matches[3][0] . '"' : '';
	return '<link rel="stylesheet" href="' . $matches[2][0] . '"' . $media . '>' . "\n";
} // mdg_clean_style_tag()
add_filter( 'style_loader_tag', 'mdg_clean_style_tag' );
 ...
 */
function mdg_embed_wrap( $cache, $url, $attr = '', $post_ID = '' ) {
	return '<div class="entry-content-asset">' . $cache . '</div>';
} // mdg_embed_wrap()
add_filter( 'embed_oembed_html', 'mdg_embed_wrap', 10, 4 );
 ...
 */
function mdg_caption( $output, $attr, $content ) {
	if ( is_feed() ) {
		return $output;
	} // if()
...
add_filter( 'img_caption_shortcode', 'mdg_caption', 10, 3 );
 ...
 * @return Void
 */
function mdg_remove_dashboard_widgets() {
	remove_meta_box( 'dashboard_incoming_links', 'dashboard', 'normal' );
	remove_meta_box( 'dashboard_plugins', 'dashboard', 'normal' );
	remove_meta_box( 'dashboard_primary', 'dashboard', 'normal' );
	remove_meta_box( 'dashboard_secondary', 'dashboard', 'normal' );
} // mdg_remove_dashboard_widgets()
add_action( 'admin_init', 'mdg_remove_dashboard_widgets' );
 ...
 * @return  Void
 */
function mdg_excerpt_length( $length ) {
	return POST_EXCERPT_LENGTH;
} // mdg_excerpt_length()
add_filter( 'excerpt_length', 'mdg_excerpt_length' );
 ...
 */
function mdg_excerpt_more( $more ) {
 ...
 */
function mdg_remove_self_closing_tags( $input ) {
	return str_replace( ' />', '>', $input );
} // mdg_remove_self_closing_tags()
add_filter( 'get_avatar',          'mdg_remove_self_closing_tags' ); // <img />
add_filter( 'comment_id_fields',   'mdg_remove_self_closing_tags' ); // <input />
add_filter( 'post_thumbnail_html', 'mdg_remove_self_closing_tags' ); // <img />
 ...
 */
function mdg_remove_default_description( $bloginfo ) {
	$default_tagline = 'Just another WordPress site';
	return ( $bloginfo === $default_tagline ) ? '' : $bloginfo;
} // mdg_remove_default_description
add_filter( 'get_bloginfo_rss', 'mdg_remove_default_description' );
 ...
 */
function mdg_nice_search_redirect() {
	global $wp_rewrite;
...
} // mdg_nice_search_redirect()
if ( current_theme_supports( 'nice-search' ) ) {
	add_action( 'template_redirect', 'mdg_nice_search_redirect' );
} // if()
 ...
 */
function mdg_request_filter( $query_vars ) {
...
add_filter( 'request', 'mdg_request_filter' );
 ...
 */
function mdg_get_search_form( $form ) {
	$form = '';
